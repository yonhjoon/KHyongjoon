...
<body>
	...
	<pre>
	*변수 선언과 동시에 초기화(c:set var="변수명" value="값"[scope="저장객채"])
	-변수 선언하고 초기값을 대입해두는 기능을 제공
	-해당 변수를 어떤 scope에 setAttribute를 통해서 담아둘지 결정할 수 있다.
	=>c:set으로 선언한 변수는 접근할 때 무조건! EL로 접근해야한다
	</pre>
	
	<c:set var="num1" value="10"/><!-- pageContext.setAttribute("num1", 10); 와 동일하다고 볼수있다 -->
	<c:set var="num2" value="20" scope="request" /> <!-- request.setAttribute("num2",20); 와 동일하다고 볼수있다 -->
	num1변수 : ${num1 } <br>
	num2변수 : ${num2 } <br>
	...
	*변수 삭제 (c:remove var="제거하고싶은 변수명" [scope=""])
	...
	request에서 삭제 후 request값 : ${request }<br>
	...
	*변수(데이터) 출력 (c:out value="출력하고자하는값" [default="기본값"] [escapeXml="true | false"])
	...
	향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자하는 객채(배열/컬렉션)"[varStatus="현재접근된 요소의 상태값"])
	</pre>
	...
	<%
		...
		pList.add(new Person("홍길동", 18, "남자"));
		pList.add(new Person("김개똥", 28, "여자"));
		pList.add(new Person("최지원", 48, "여자"));
	%>
	...
	<table>
		...
		<%--
			<% if(list.isEmpty()){ %>
				<tr><td colspan="3">조회된 사람이 없습니다.</tr>
			<%} else{%>
				<%for(Person p : list){ %>
					<tr>
						<td><%p.getName() %></td>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="p" items="${pList }">
						<tr>
							<td>${status.count }</td> <!-- 순차적을 번호를 매기고싶을까 -->
							<td>${p.name }</td>
							<td>${p.age }</td>
							<td>${p.gender }</td>
						</tr>
