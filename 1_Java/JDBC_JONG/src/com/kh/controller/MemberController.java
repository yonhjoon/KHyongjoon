...
public class MemberController {
	 ...
	 * */
	public void insertMember(String userId, String userPwd, String userName, String email, String phone, String address, String hobby) {
		...
		Member m = new Member(userId, userPwd, userName, email, phone, address, hobby); 
		
		int result = new MemberService().insertMember(m);
		
		if (result > 0) { // insert가 성공적으로 완료되었다
			//성공화면
			new MemberMenu().displaySuccess("성공적으로"+ m.getUserName() +" 화가님의 그림이 추가되었습니다.");
		} else { // insert가 실패했다.
			//실패화면
			new MemberMenu().displayFail(m.getUserName()+"화가님의 그림추가를 실패하였습니다.");
		}
	}
	...
	public void updateMember(String userId,String userPwd,
		...
		if (result > 0) {
			new MemberMenu().displaySuccess("성공적으로 그림의정보가 변경되었습니다.");
		} else {
			new MemberMenu().displayFail("그림정보 변경에 실패하였습니다.");
		}
	}
	...
	public void deleteMember(String userId) {
		...
		if (result > 0) {
			new MemberMenu().displaySuccess("성공적으로 그림정보가 삭제되었습니다.");
		} else {
			new MemberMenu().displayFail("그림정보 삭제에 실패하였습니다.");
		}
	}
